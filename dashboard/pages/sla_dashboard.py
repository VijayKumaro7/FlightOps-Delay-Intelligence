...
from config import (
    DELAY_CRITICAL_PCT, DELAY_HIGH_PCT,
    SLA_TARGET_PCT, SLA_AT_RISK_PCT,
    CHRONIC_WINDOW_DAYS, CHRONIC_MIN_BREACHES,
    SLA_WINDOW_DAYS,
)
...
SELECT
    ...
    bl.delayed_flights,
    bl.delay_rate_pct,
    :sla_threshold                                                       AS sla_threshold_pct,
    ROUND(bl.delay_rate_pct - :sla_threshold, 2)                        AS overage_pct,
    CASE
        WHEN bl.delay_rate_pct >= {critical_pct} THEN 'CRITICAL'
        WHEN bl.delay_rate_pct >= {high_pct}     THEN 'HIGH'
        ELSE                                          'MEDIUM'
    END                                                                  AS severity,
...
WHERE bl.resolved = 0
  AND bl.delay_rate_pct > :sla_threshold
  {carrier_filter}
...
WITH breach_counts AS (
    ...
    FROM  sla_breach_log
    WHERE breach_date >= DATE_SUB(CURDATE(), INTERVAL {window_days} DAY)
    GROUP BY route_id
    HAVING breach_count >= {min_breaches}
)
SELECT
    bc.breach_count,
    CONCAT(r.origin, ' → ', r.destination)                              AS route,
...
breached_routes AS (
    ...
    FROM  sla_breach_log bl
    JOIN  routes         r ON bl.route_id = r.route_id
    WHERE bl.breach_date >= DATE_SUB(CURDATE(), INTERVAL {window_days} DAY)
      AND bl.delay_rate_pct > :sla_threshold
)
SELECT
    a.carrier_name,
...
def render(selected_carrier: str, start_date, end_date, sla_threshold: float = 15.0):
    params = {"sla_threshold": sla_threshold}
    ...
    with st.spinner("Loading breach data..."):
        df_breach = run_query(
            _ACTIVE_BREACHES_SQL.format(
                carrier_filter=cf,
                critical_pct=DELAY_CRITICAL_PCT,
                high_pct=DELAY_HIGH_PCT,
            ),
            params,
        )

    if df_breach.empty:
        st.success(f"No active SLA breaches above {sla_threshold}% threshold.")
    else:
        ...
        fig_sev = px.pie(
            ...
            hole=0.45, title="Breach Severity Distribution",
        )
        st.plotly_chart(fig_sev, use_container_width=True)

        def _sev_style(val):
            styles = {"CRITICAL": "background-color:#fde8e8",
        ...
        styled = df_breach[display_cols].style.applymap(_sev_style, subset=["severity"])
        st.dataframe(styled, use_container_width=True)
        st.download_button("⬇ Download Active Breaches CSV",
                           df_breach[display_cols].to_csv(index=False),
                           file_name="active_breaches.csv", mime="text/csv")

    st.divider()

    # ── Chronic Offenders ─────────────────────────────────────────────────────
    st.subheader(f"Chronic Offenders (Last {CHRONIC_WINDOW_DAYS} Days)")
    with st.spinner("Loading chronic offenders..."):
        df_chronic = run_query(
            _CHRONIC_SQL.format(
                carrier_filter=cf,
                window_days=CHRONIC_WINDOW_DAYS,
                min_breaches=CHRONIC_MIN_BREACHES,
            ),
            params,
        )

    if df_chronic.empty:
        st.info(f"No routes with {CHRONIC_MIN_BREACHES}+ breaches in the last "
                f"{CHRONIC_WINDOW_DAYS} days.")
    else:
        fig_chronic = px.bar(
            df_chronic.sort_values("breach_count", ascending=True),
            x="breach_count", y="route",
            color="pattern", color_discrete_map=_PATTERN_COLORS,
            orientation="h",
            labels={"breach_count": f"Breach Count (last {CHRONIC_WINDOW_DAYS} days)",
                    "route": "Route"},
            title="Chronic Offender Routes",
            hover_data={"carrier_name": True, "avg_delay_rate": ":.1f",
                        "peak_delay_rate": ":.1f"},
            text="breach_count",
        )
        fig_chronic.update_traces(textposition="outside")
        fig_chronic.update_layout(height=max(350, len(df_chronic) * 36))
        st.plotly_chart(fig_chronic, use_container_width=True)

        with st.expander("Chronic offenders table"):
            st.dataframe(df_chronic, use_container_width=True)
            st.download_button("⬇ Download Chronic Offenders CSV",
                               df_chronic.to_csv(index=False),
                               file_name="chronic_offenders.csv", mime="text/csv")

    st.divider()

    # ── Carrier SLA Compliance ────────────────────────────────────────────────
    st.subheader(f"Carrier SLA Compliance (Last {SLA_WINDOW_DAYS} Days, threshold {sla_threshold}%)")
    with st.spinner("Loading compliance data..."):
        df_comp = run_query(
            _COMPLIANCE_SQL.format(
                carrier_filter=cf,
    ...
    if df_comp.empty:
        st.info("No compliance data available for the selected filters.")
        return
    ...
    for i, row in enumerate(df_comp.itertuples()):
        ...
        color = "#27ae60" if pct >= SLA_TARGET_PCT else \
                "#e67e22" if pct >= SLA_AT_RISK_PCT else "#e74c3c"
        fig_g = go.Figure(go.Indicator(
            ...
            gauge={
                ...
                "bar": {"color": color},
                "steps": [
                    {"range": [0, SLA_AT_RISK_PCT],  "color": "#fde8e8"},
                    {"range": [SLA_AT_RISK_PCT, SLA_TARGET_PCT], "color": "#fef3e2"},
                    {"range": [SLA_TARGET_PCT, 100], "color": "#eafaf1"},
                ],
    ...
    fig_comp = px.bar(
        ...
        x="sla_compliance_pct", y="carrier_name", orientation="h",
        ...
        title=f"SLA Compliance Rate by Carrier (threshold {sla_threshold}%)",
        text="sla_compliance_pct",
    )
    fig_comp.update_traces(texttemplate="%{text:.1f}%", textposition="outside")
    fig_comp.add_vline(x=SLA_TARGET_PCT, line_dash="dash", line_color="green",
                       annotation_text=f"{SLA_TARGET_PCT:.0f}% target")
    fig_comp.add_vline(x=SLA_AT_RISK_PCT, line_dash="dash", line_color="orange",
                       annotation_text=f"{SLA_AT_RISK_PCT:.0f}% at-risk")
    fig_comp.update_layout(coloraxis_showscale=False, height=420)
    st.plotly_chart(fig_comp, use_container_width=True)

    with st.expander("Compliance data table"):
        st.dataframe(df_comp, use_container_width=True)
        st.download_button("⬇ Download Compliance CSV",
                           df_comp.to_csv(index=False),
                           file_name="sla_compliance.csv", mime="text/csv")
